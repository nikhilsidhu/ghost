...
pub type IdlogResult<T> = std::result::Result<T, IdlogError>;
...
pub fn encode_body(buf: &mut Vec<u8>, body: &EntryBody) {
    match body {
...
/// Build the message that gets signed: prefix + canonical bytes.
pub fn sign_message(entry: &LogEntry) -> Vec<u8> {
...
fn verify_sig(key_bytes: &[u8; 32], entry: &LogEntry) -> IdlogResult<()> {
    let key = VerifyingKey::from_bytes(key_bytes)
        .map_err(|e| IdlogError::InvalidKey(format!("bad verifying key: {e}")))?;
    let sig = Signature::from_bytes(&entry.signature);
    key.verify_strict(&sign_message(entry), &sig)
        .map_err(|_| IdlogError::AuthenticationFailed)
}

fn verify_counter_sig(key_bytes: &[u8; 32], entry: &LogEntry) -> IdlogResult<()> {
    let cs = entry.counter_signature
        .ok_or_else(|| IdlogError::Format("missing counter-signature".into()))?;
    let key = VerifyingKey::from_bytes(key_bytes)
        .map_err(|e| IdlogError::InvalidKey(format!("bad verifying key: {e}")))?;
...
pub fn validate_chain(entries: &[LogEntry]) -> IdlogResult<LogState> {
    if entries.is_empty() {
        return Err(IdlogError::Format("empty identity log".into()));
    }
...
pub fn validate_entry(state: &mut LogState, entry: &LogEntry) -> IdlogResult<()> {
    // Seq must be exactly next
    if entry.seq != state.head_seq + 1 {
        return Err(IdlogError::Format(format!(
    ...
    if entry.seq == 1 {
        if entry.prev_hash != [0u8; 32] {
            return Err(IdlogError::Format("idlog: genesis prev_hash must be zeroed".into()));
        }
    } else if entry.prev_hash != state.head_hash {
        return Err(IdlogError::Format("idlog: prev_hash mismatch".into()));
    }

    // Account fingerprint must be consistent
    if entry.account_fp != state.account_fp {
        return Err(IdlogError::Format("idlog: account_fp mismatch".into()));
    }

    match &entry.body {
        EntryBody::Genesis { master_verifying_key, device_verifying_key, device_label } => {
            if entry.seq != 1 {
                return Err(IdlogError::Format("idlog: genesis must be seq 1".into()));
            }
            let expected_fp: [u8; 32] = blake3::hash(master_verifying_key).into();
            if expected_fp != state.account_fp {
                return Err(IdlogError::Format("idlog: fingerprint doesn't match master key".into()));
            }
        ...
        EntryBody::AddDevice { device_verifying_key, device_label, authorizer_key } => {
            let master = state.master_verifying_key
                .ok_or_else(|| IdlogError::Format("idlog: AddDevice before genesis".into()))?;

            if *authorizer_key != master && !state.is_active_device(authorizer_key) {
                return Err(IdlogError::Format("idlog: unauthorized signer".into()));
            }
            verify_sig(authorizer_key, entry)?;
            verify_counter_sig(device_verifying_key, entry)?;

            if state.devices.contains_key(device_verifying_key) {
                return Err(IdlogError::Format("idlog: duplicate device key".into()));
            }
        ...
        EntryBody::RevokeDevice { device_verifying_key, revoker_key } => {
            let master = state.master_verifying_key
                .ok_or_else(|| IdlogError::Format("idlog: RevokeDevice before genesis".into()))?;

            if *revoker_key != master && !state.is_active_device(revoker_key) {
                return Err(IdlogError::Format("idlog: unauthorized revoker".into()));
            }
            if revoker_key == device_verifying_key {
                return Err(IdlogError::Format("idlog: cannot self-revoke".into()));
            }
            if !state.is_active_device(device_verifying_key) {
                return Err(IdlogError::Format("idlog: device not active".into()));
            }
        ...
        EntryBody::Recovery { device_verifying_key, device_label } => {
            let master = state.master_verifying_key
                .ok_or_else(|| IdlogError::Format("idlog: Recovery before genesis".into()))?;
...
impl LogEntry {
    ...
    pub fn from_bytes(data: &[u8]) -> IdlogResult<Self> {
        let err = |msg: &str| IdlogError::Format(format!("idlog: {msg}"));
...
fn decode_body(entry_type: EntryType, data: &[u8]) -> IdlogResult<EntryBody> {
    let err = |msg: &str| IdlogError::Format(format!("idlog body: {msg}"));
...
mod tests {
    ...
    fn create_genesis(mk: &SigningKey, dk: &SigningKey, label: &str) -> LogEntry {
    ...
    fn create_add_device(state: &LogState, auth: &SigningKey, new: &SigningKey, label: &str) -> LogEntry {
        let mut entry = LogEntry {
            seq: state.head_seq + 1,
            prev_hash: state.head_hash,
            account_fp: state.account_fp,
            entry_type: EntryType::AddDevice,
            timestamp: 1000 + state.head_seq * 100,
        ...
        sign_entry(auth, &mut entry);
        counter_sign(new, &mut entry);
        entry
    }

    fn create_revoke_device(state: &LogState, revoker: &SigningKey, target: &[u8; 32]) -> LogEntry {
        let mut entry = LogEntry {
            seq: state.head_seq + 1,
            prev_hash: state.head_hash,
            account_fp: state.account_fp,
            ...
            body: EntryBody::RevokeDevice {
                device_verifying_key: *target,
                revoker_key: revoker.verifying_key().to_bytes(),
            },
            signature: [0u8; 64],
            counter_signature: None,
        };
        sign_entry(revoker, &mut entry);
        entry
    }

    fn create_recovery(state: &LogState, mk: &SigningKey, new: &SigningKey, label: &str) -> LogEntry {
        let mut entry = LogEntry {
            seq: state.head_seq + 1,
            prev_hash: state.head_hash,
            account_fp: state.account_fp,
